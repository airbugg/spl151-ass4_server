...
public class ConnectionHandler<T> {
	...
	private static final Logger logger = Logger.getLogger("edu.spl.reactor_server.reactor");
	...
	public void read() throws IOException {
		// do not read if reactor_protocol has terminated. only write of pending data is
		// allowed
		...
		if (numBytesRead == -1) {
			...
			closeConnection();
			// tell the reactor_protocol that the connection terminated.
			_protocol.connectionTerminated();
			return;
		}

		//add the buffer to the reactor_protocol task
		buf.flip();
		_task.addBytes(buf);
		// add the reactor_protocol task to the reactor_server.reactor
	...
	public synchronized void write() throws IOException, ClosedChannelException{
		...
		// check if the reactor_protocol indicated close.
		if (_protocol.shouldClose()) {
